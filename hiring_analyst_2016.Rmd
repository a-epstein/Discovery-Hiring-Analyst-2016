...
# Remove lengths of 0 and lengths greater than the 99th percentile
...
sessions <- events %>%
  ...
  mutate(len = max - min) %>%
  filter(len != 0, len < quantile(sessions$len, 0.99))

# Box plot comparing groups a and b session length variable

box <- ggplot(sessions, aes(factor(group), len)) + geom_boxplot(aes(fill = factor(group)), width=0.5, outlier.colour = "dodgerblue", outlier.size = 1, outlier.shape = 16, outlier.stroke = 1) + labs(title="Session length in group a vs group b", x="Group", y="Session Length (s)")
